...
const searchFormStore = Store.namespace('searchForm')
const tabKey = Store.namespace('tabKey')
export default {
  name: 'ListPageTpl',
  components: {
    SearchForm,
  ...
  data () {
    ...
    return {
      curTab: this.remember && tabKey.session(name) ? tabKey.session(name) : this.getActiveTab(),
      searchForm: this.remember && searchFormStore.session(name) ? searchFormStore.session(name) : {},
  ...
  computed: {
    ...
    currentTabKey () {
      tabKey.session(this.storeKey, this.curTab)
  ...
  methods: {
    ...
    formSearching (formData) {
      ...
      searchFormStore.session(name, cloneJsonObj(this.searchForm))
    }
