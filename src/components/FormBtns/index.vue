<template>
  <div class="lcomp-form-btns">
    <el-button type="primary" @click="submitHandle" :loading="sending">{{sending ? handleWords.sending : handleWords.sureBtn}}</el-button>
    <template v-if="!submitOnly">
      <el-button v-if="isEdit" @click="callMethod('cancel')">{{handleWords.cancelBtn}}</el-button>
      <el-button v-else @click="resetForm">{{handleWords.resetBtn}}</el-button>
    </template>
  </div>
</template>
<script>
