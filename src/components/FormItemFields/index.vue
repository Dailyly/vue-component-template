...
export default {
  ...
  data () {
    return {
      handleFieldItems: cloneDeep(this.fieldItems.filter(item => (!item.disabled)))
    }
  },
  computed: {
    handleValue: {
  ...
  watch: {
    fieldItems: {
      handler () {
        this.handleFieldItems = cloneDeep(this.fieldItems.filter(item => (!item.disabled)))
        this.watchModel()
        this.resetFieldData()
      },
      deep: true
  ...
  methods: {
    ...
    setValue (prop, $event) {
      prop ? (this.handleValue[prop] = $event) : (this.handleValue = { ...this.handleValue, [prop]: $event })
    },
    // model值动态watch
    watchModel () {
